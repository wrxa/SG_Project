...
class CoalCHPDesulfurization(db.Model):
    ...
    plan_id = db.Column(db.Integer, db.ForeignKey('plan.id'))
    # 收到基硫份设计值
    s_sulfur_design_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 1#收到基硫份:校核值
    s_sulfur_design_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 计算耗煤量设计值
    s_calcu_coal_consume_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 2#计算耗煤量:校核值
    s_calcu_coal_consume_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 燃煤中的含硫量燃烧后氧化成SO2的份额设计值
    s_aflame_generate_so2_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 3#燃煤中的含硫量燃烧后氧化成SO2的份额:校核值
    s_aflame_generate_so2_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 脱硫前烟气中的SO2含量设计值
    s_desulfrization_before_so2_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 4#脱硫前烟气中的SO2含量:校核值
    s_desulfrization_before_so2_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 引风机进口烟气容积流量（标况）设计值
    s_fan_smoke_flow_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 5#引风机进口烟气容积流量（标况）:校核值
    s_fan_smoke_flow_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 未脱硫前SO2浓度（标态）设计值
    s_no_desulfurization_so2_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 6#未脱硫前SO2浓度（标态）:校核值
    s_no_desulfurization_so2_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 脱硫效率设计值
    s_desulfurization_efficiency_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 7#脱硫效率:校核值
    s_desulfurization_efficiency_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 脱硫后SO2浓度（标态）设计值
    s_desulfrization_after_so2_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 8#脱硫后SO2浓度（标态）:校核值
    s_desulfrization_after_so2_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 脱硫后SO2排放量（标态）设计值
    s_desulfrization_after_discharge_so2_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 9#脱硫后SO2排放量（标态）:校核值
    s_desulfrization_after_discharge_so2_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 炉内脱硫百分比设计值
    r_furnace_rate_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 10#炉内脱硫百分比:校核值
    r_furnace_rate_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 炉内脱硫后SO2浓度设计值
    r_furnace_concentration_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 11#炉内脱硫后SO2浓度:校核值
    r_furnace_concentration_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 脱除SO2质量设计值
    r_others_mass_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 12#脱除SO2质量:校核值
    r_others_mass_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 脱除SO2摩尔量设计值
    r_others_mole_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 13#脱除SO2摩尔量:校核值
    r_others_mole_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 钙硫摩尔比设计值
    r_calcium_sulfur_rate_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 14#钙硫摩尔比:校核值
    r_calcium_sulfur_rate_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 反应所需CaCO3摩尔量设计值
    r_nees_caco3_mole_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 15#反应所需CaCO3摩尔量:校核值
    r_nees_caco3_mole_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 反应所需CaCO3质量设计值
    r_nees_caco3_mass_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 16#反应所需CaCO3质量:校核值
    r_nees_caco3_mass_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 参加反应CaCO3质量设计值
    r_use_caco3_mass_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 17#参加反应CaCO3质量:校核值
    r_use_caco3_mass_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 反应生成CaSO4质量设计值
    r_generate_coco3_mass_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 18#反应生成CaSO4质量:校核值
    r_generate_coco3_mass_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 反应后质量增加设计值
    r_add_mass_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 19#反应后质量增加:校核值
    r_add_mass_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石纯度设计值
    r_caco3_pure_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 20#石灰石纯度:校核值
    r_caco3_pure_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石耗量设计值
    r_coco3_consume_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 21#石灰石耗量:校核值
    r_coco3_consume_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 炉内脱硫产生的灰渣量设计值
    r_generate_grey_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 22#炉内脱硫产生的灰渣量:校核值
    r_generate_grey_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石粉仓储存时间设计值
    r_storage_time_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 23#石灰石粉仓储存时间:校核值
    r_storage_time_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石粉仓出力设计值
    r_storage_output_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 24#石灰石粉仓出力:校核值
    r_storage_output_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石粉堆积密度设计值
    r_storage_density_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 25#石灰石粉堆积密度:校核值
    r_storage_density_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石粉库充满系数设计值
    r_storage_fullness_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 26#石灰石粉库充满系数:校核值
    r_storage_fullness_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石粉仓体积设计值
    r_storage_volume_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 27#石灰石粉仓体积:校核值
    r_storage_volume_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 高设计值
    r_height_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 28#高:校核值
    r_height_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 直径设计值
    r_diameter_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 29#直径:校核值
    r_diameter_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 石灰石纯度设计值
    d_limestone_pure_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 30#石灰石纯度:校核值
    d_limestone_pure_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # Ca/S（钙硫比）设计值
    d_proportion_ca_s_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 31#Ca/S（钙硫比）:校核值
    d_proportion_ca_s_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 脱硫效率设计值
    d_desulfurization_efficiency_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 32#脱硫效率:校核值
    d_desulfurization_efficiency_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 石灰石消耗量设计值
    d_limestone_consume_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 33#石灰石消耗量:校核值
    d_limestone_consume_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 生成CaSO4量设计值
    d_gengrate_coca4_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 34#生成CaSO4量:校核值
    d_gengrate_coca4_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 脱硝前NOX浓度设计值
    n_before_nox_concentration_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 35#脱硝前NOX浓度:校核值
    n_before_nox_concentration_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 引风机进口烟气容积流量（标况）设计值
    n_input_smoke_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 36#引风机进口烟气容积流量（标况）:校核值
    n_input_smoke_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 脱硝效率(总效率)设计值
    n_desulfurization_efficiency_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 37#脱硝效率(总效率):校核值
    n_desulfurization_efficiency_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 脱硝前NOX排放量设计值
    n_before_nox_discharge_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 38#脱硝前NOX排放量:校核值
    n_before_nox_discharge_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 脱硝后NOX浓度设计值
    n_after_nox_concentration_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 39#脱硝后NOX浓度:校核值
    n_after_nox_concentration_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 环保要求NOX的排放浓度设计值
    n_env_after_nox_concentration_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 40#环保要求NOX的排放浓度:校核值
    n_env_after_nox_concentration_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 脱硝后NOX排放量设计值
    n_after_nox_discharge_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 41#脱硝后NOX排放量:校核值
    n_after_nox_discharge_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 炉内脱硝百分比设计值
    d_denitration_percentage_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 42#炉内脱硝百分比:校核值
    d_denitration_percentage_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 炉内脱硝量设计值
    d_denitration_quality_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 43#炉内脱硝量:校核值
    d_denitration_quality_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 炉内脱硝后NOX排放量设计值
    d_after_nox_discharge_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 44#炉内脱硝后NOX排放量:校核值
    d_after_nox_discharge_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 炉内脱硝摩尔量设计值
    d_denitration_molar_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 45#炉内脱硝摩尔量:校核值
    d_denitration_molar_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 氨逃逸率设计值
    d_escape_rate_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 46#氨逃逸率:校核值
    d_escape_rate_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 氨逃逸量设计值
    d_escape_quality_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 47#氨逃逸量:校核值
    d_escape_quality_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 逃逸氨折算尿素量设计值
    d_escape_quality_urea_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 48#逃逸氨折算尿素量:校核值
    d_escape_quality_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # NH3/NOX摩尔比设计值
    d_nh3nox_molar_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 49#NH3/NOX摩尔比:校核值
    d_nh3nox_molar_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 尿素/NOX摩尔比设计值
    d_urea_nox_molar_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 50#尿素/NOX摩尔比:校核值
    d_urea_nox_molar_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 尿素/NOX式量比设计值
    d_urea_nox_quality_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 51#尿素/NOX式量比:校核值
    d_urea_nox_quality_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 理论尿素消耗量设计值
    d_theory_urea_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 52#理论尿素消耗量:校核值
    d_theory_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 尿素用量(一台炉)设计值
    d_use_urea_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 53#尿素用量(一台炉):校核值
    d_use_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 尿素溶液消耗水量(一台炉)设计值
    d_water_urea_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 54#尿素溶液消耗水量(一台炉):校核值
    d_water_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 尿素仓库天数设计值
    d_days_urea_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 55#尿素仓库天数:校核值
    d_days_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 尿素仓库容量设计值
    d_capacity_urea_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 56#尿素仓库容量:校核值
    d_capacity_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 烟气脱硝百分比设计值
    g_denitration_percentage_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 57#烟气脱硝百分比:校核值
    g_denitration_percentage_check = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 烟气脱硝后NOX排放量设计值
    g_after_nox_discharge_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 58#烟气脱硝后NOX排放量:校核值
    g_after_nox_discharge_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 烟气脱硝量设计值
    g_denitration_quality_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 59#烟气脱硝量:校核值
    g_denitration_quality_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 氨逃逸率设计值
    g_escape_rate_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 60#氨逃逸率:校核值
    g_escape_rate_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 氨逃逸量设计值
    g_escape_quality_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 61#氨逃逸量:校核值
    g_escape_quality_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # 逃逸氨折算尿素量设计值
    g__escape_quality_urea_design = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 62#逃逸氨折算尿素量:校核值
    g__escape_quality_urea_check = db.Column(db.NUMERIC(precision=15, scale=5))
    # NH3/NOX摩尔比设计值
    d_nh3nox_molar_design = db.Column(db.NUMERIC(precision=15, scale=5))
    # 63#NH3/NOX摩尔比:校核值
    d_nh3nox_molar_check = db.Column(db.NUMERIC(precision=15, scale=5))
...
class CoalCHPBoilerAuxiliaries(db.Model):
    ...
    # 水处理设备全部出力
    m_water_equipment_output = db.Column(db.NUMERIC(precision=15, scale=5))
    # 选取水处理设备出力
    m_water_equipment_select = db.Column(db.NUMERIC(precision=15, scale=5))
...
class CoalCHPSmokeAirSystem(db.Model):
    ...
    # 工况温度
    b_temperature_case = db.Column(db.NUMERIC(precision=15, scale=5))
    # 工况流量
    b_operational_point_flowrate = db.Column(db.NUMERIC(precision=15, scale=5))
    # 当地大气压
    b_local_atmosphere = db.Column(db.NUMERIC(precision=15, scale=5))
    # 标况温度
    b_the_case_temperature = db.Column(db.NUMERIC(precision=15, scale=5))
    # 标况压力
    b_standard_of_pressure = db.Column(db.NUMERIC(precision=15, scale=5))
    # 标况流量
    b_standard_of_flow = db.Column(db.NUMERIC(precision=15, scale=5))

    # 标况温度
    p_the_case_temperature = db.Column(db.NUMERIC(precision=15, scale=5))
    # 标况压力
    p_standard_of_pressure = db.Column(db.NUMERIC(precision=15, scale=5))
    # 标况流量
    p_standard_of_flow = db.Column(db.NUMERIC(precision=15, scale=5))
    # 工况温度
    p_temperature_case = db.Column(db.NUMERIC(precision=15, scale=5))
    # 当地大气压
    p_local_atmosphere = db.Column(db.NUMERIC(precision=15, scale=5))
    # 工况流量
    p_operational_point_flowrate = db.Column(db.NUMERIC(precision=15, scale=5))
...
class CoalCHPCirculatingWater(db.Model):
    ...
    # 辅机冷却水量
    v_auxiliary_engine_cooling_water = db.Column(
        db.NUMERIC(precision=15, scale=5))
    # 总循环水量(冬季)
    ...
    # 分吹损失
    v_points_blow_loss = db.Column(db.NUMERIC(precision=15, scale=5))
    # 浓缩倍率
