...
class Smoke_heat_efficiency(FieldCalculation):
    def notify(self, val):
        result = val['dbresult']
        if val['condensate_backwater_pressure'] != '' and val['condensate_backwater_pressure'] is not None and val['t_point_enthalpy'] != '' and val['t_point_enthalpy'] is not None and val['t_point_flow'] != '' and val['t_point_flow'] is not None and val['t_total_power'] != '' and val['t_total_power'] is not None and val['t_base_heat_received_user'] != '' and val['t_base_heat_received_user'] is not None and val['t_rated_fuel_consumption'] != '' and val['t_rated_fuel_consumption'] is not None:
            smoke_heat_efficiency = ((float(val['t_total_power']))*3.6+(float(val['t_point_flow']))*((float(val['t_point_enthalpy']))-(seuif97.HL_P(10.00*(float(val['condensate_backwater_pressure']))))))/(float(val['t_rated_fuel_consumption']))/(float(val['t_base_heat_received_user']))*100
...
class Heat_efficiency(FieldCalculation):
    def notify(self, val):
        result = val['dbresult']
        if val['t_total_power0'] != '' and val['t_total_power0'] is not None and val['t_base_heat_received_user'] != '' and val['t_base_heat_received_user'] is not None and val['t_rated_fuel_consumption'] != '' and val['t_rated_fuel_consumption'] is not None:
            heat_efficiency = (float(val['t_total_power0']))*3.6/(float(val['t_rated_fuel_consumption']))/(float(val['t_base_heat_received_user']))*100
