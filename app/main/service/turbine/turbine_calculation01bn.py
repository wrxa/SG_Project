...
# 实现字段i_steam_exhaust_pressure:144背压压力,的计算37
...
# 实现字段i_steam_exhaust_enthalpy:特殊处理部分--146焓,的计算39
...
# 实现字段i_steam_exhaust_enthalpy_actual:147实际焓,的计算40
...
# 实现字段i_steam_exhaust_enthalpy_steam:特殊处理部分--148饱和蒸汽焓,的计算41
...
# 实现字段i_steam_exhaust_enthalpy_water:特殊处理部分--149饱和水焓,的计算42
...
# 实现字段i_steam_exhaust_dry:150干度,的计算43
...
# 实现字段i_steam_exhaust_flow:151流量,的计算44
class I_steam_exhaust_flow(FieldCalculation):
    def notify(self, val):
        result = val['dbresult']
        if val['h_temperature'] != '' and val['h_temperature'] is not None and val['d_water_temperature'] != '' and val['d_water_temperature'] is not None and val['d_work_pressure'] != '' and val['d_work_pressure'] is not None and val['d_pressure_loss'] != '' and val['d_pressure_loss'] is not None and val['lh1_top_difference'] != '' and val['lh1_top_difference'] is not None and val['lh1_pressure_loss'] != '' and val['lh1_pressure_loss'] is not None and val['e_exhaust_point_flow'] != '' and val['e_exhaust_point_flow'] is not None and val['s_lh_grade'] != '' and val['s_lh_grade'] is not None and val['h_blowdown_rate'] != '' and val['h_blowdown_rate'] is not None and val['e_steam_exhaust_pressure'] != '' and val['e_steam_exhaust_pressure'] is not None and val['e_steam_water_loss'] != '' and val['e_steam_water_loss'] is not None and val['e_throttle_flow'] != '' and val['e_throttle_flow'] is not None and val['i_steam_flow'] != '' and val['i_steam_flow'] is not None and val['e_steam_pressure'] != '' and val['e_steam_pressure'] is not None and val['i_high1_flow'] != '' and val['i_high1_flow'] is not None and val['e_steam_temperature'] != '' and val['e_steam_temperature'] is not None and val['i_high2_flow'] != '' and val['i_high2_flow'] is not None and val['i_low2_flow'] != '' and val['i_low2_flow'] is not None:
...
# 实现字段i_total_power:153总功率,的计算45
...
# 实现字段i_calculation_error:154计算误差---抽凝,的计算46
...
# 实现字段h_enthalpy:特殊处理部分--28焓值,的计算48
...
# 实现字段hh1_water_temperature:30给水出水温度,的计算50
...
# 实现字段hh1_water_enthalpy:特殊处理部分--31给水出口焓,的计算51
...
# 实现字段hh1_top_difference:32上端差,的计算52
...
# 实现字段hh1_saturated_water_temperature:33饱和水温度,的计算53
...
# 实现字段hh1_saturated_water_enthalpy:特殊处理部分--34饱和水焓,的计算54
...
# 实现字段hh1_work_pressure:特殊处理部分--35工作压力,的计算55
...
# 实现字段hh1_pressure_loss:36抽汽管压损,的计算56
...
# 实现字段hh1_extraction_pressure:37抽汽压力,的计算57
...
# 实现字段hh1_extraction_enthalpy:特殊处理部分--38抽汽焓,的计算58
...
# 实现字段hh1_extraction_amount:39抽汽量,的计算59
...
# 实现字段d_water_enthalpy:特殊处理部分--51给水出口焓,的计算60
...
# 实现字段d_extraction_pressure:54抽汽压力,的计算61
...
# 实现字段d_extraction_enthalpy:特殊处理部分--55抽汽焓,的计算62
...
# 实现字段d_extraction_amount:56抽汽量,的计算63
...
# 实现字段lh1_water_temperature:57给水出水温度,的计算64
...
# 实现字段lh1_water_enthalpy:58给水出口焓,的计算65
...
# 实现字段lh1_saturated_water_temperature:60饱和水温度,的计算66
...
# 实现字段lh1_saturated_water_enthalpy:特殊处理部分--61饱和水焓,的计算67
...
# 实现字段lh1_work_pressure:特殊处理部分--62工作压力,的计算68
...
# 实现字段lh1_extraction_pressure:64抽汽压力,的计算69
...
# 实现字段lh1_extraction_enthalpy:特殊处理部分--65抽汽焓,的计算70
...
# 实现字段lh1_extraction_amount:66抽汽量,的计算71
...
# 实现字段c_water_temperature:特殊处理部分--77给水出水温度,的计算72
...
# 实现字段c_water_enthalpy:特殊处理部分--78给水出口焓,的计算73
...
# 实现字段c_work_pressure:79背压工作压力,的计算74
