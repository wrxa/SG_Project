...
function drawEquipmentList(data){
    ...
    for(var i=0; i<titlelist.length; i++){
        tbodyString += analyzeEquipmentType($.parseJSON(equipment_json), titlelist[i].key, titlelist[i].val, id);
        id += 1;
    }
    ...
    var tbodyString = "<tr attrid='" + type + "' class='alert alert-success alert-dismissible' role='alert'>"
        ...
        for(var j = 0; j < equipmentCount; j++){
            if(equipment_json.equipment_type[j] == type){
                ...
                + "<textarea style='resize:none;height: 40px;' onkeydown='checkEnter(event)' wrap='soft' class='form-control name' name='equipmentName_" + j + "' id='equipmentName_" + j + "'>" + equipment_json.equipment_name[j] + "</textarea>"
                ...
                + "<textarea style='resize:none;height: 40px;' onkeydown='checkEnter(event)' wrap='soft' class='form-control content' name='equipmentContent_" + j + "' id='equipmentContent_" + j + "'>" + equipment_json.equipment_content[j] + "</textarea>"
                ...
                + "<textarea style='resize:none;height: 40px;' onkeydown='checkEnter(event)' wrap='soft' class='form-control remark' name='equipmentRemark_" + j + "' id='equipmentRemark_" + j + "'>" + equipment_json.equipment_remark[j] + "</textarea>"
                +"</td>";
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.state == 'success'){
                getEquipmentList();
                if(flag == 0){
                    messageToast('success', '燃气蒸汽联合循环-设备清单删除成功',2000);
                }else{
                    messageToast('success', '燃气蒸汽联合循环-设备清单保存成功',2000);
                }             
            }else{
                if(flag == 0){
                    messageToast('error', '燃气蒸汽联合循环-设备清单删除失败',2000);
                }else{
                    messageToast('info', '燃气蒸汽联合循环-设备清单保存失败',2000);
                }
            }
