...
$(function() {
    ...
    $("#submitData").click(function(){
        //单位处理：单压，双压锅炉效率/100, 双压的排污率/100
        // $("#sp_boiler_efficiency_design").val($("#sp_boiler_efficiency_design").val()/100)
        // $("#high_boiler_efficiency_design").val($("#high_boiler_efficiency_design").val()/100)
        // $("#high_blowdown_rate_design").val($("#high_blowdown_rate_design").val()/100)
        $("#ccpptargethtml").val('ccppcalculate')
        $.ajax({
            cache: false,
            type: "POST",
    ...
    // 选择燃机
    $("#engine_id_design").change(function(){
        var deviceid = $(this).val();
        if(deviceid == '-1'){
            $('#engine_num_design').val("")
            $('#engine_model_design').val("")
            $('#engine_power_design').val("")
            $('#engine_heat_consmption_rate_design').val("")
            $('#engine_efficiency_design').val("")
            $('#engine_pressure_ratio_design').val("")
            $('#individual_gas_consumption_design').val("")
            $('#engine_exhuast_gas_temperature_design').val("")
            $('#engine_exhuast_gas_flux_design').val("")
            return
        }
        $.ajax({
            cache: true,
            type: "POST",
            url: './findgasTurbineById',
            data: { 
                'deviceid':deviceid
            },
            async: false,
            ...
            success: function (data) {
                $('#engine_model_design').val(data.ccpp_ccpp['engine_model_design'])
                $('#engine_power_design').val(data.ccpp_ccpp['engine_power_design'])
                $('#engine_heat_consmption_rate_design').val(data.ccpp_ccpp['engine_heat_consmption_rate_design'])
                $('#engine_efficiency_design').val(data.ccpp_ccpp['engine_efficiency_design'])
                $('#engine_pressure_ratio_design').val(data.ccpp_ccpp['engine_pressure_ratio_design'])
                $('#individual_gas_consumption_design').val(data.ccpp_ccpp['individual_gas_consumption_design'])
                $('#engine_exhuast_gas_temperature_design').val(data.ccpp_ccpp['engine_exhuast_gas_temperature_design'])
                $('#engine_exhuast_gas_flux_design').val(data.ccpp_ccpp['engine_exhuast_gas_flux_design'])
            }
        });
    })
    ...
    $('.btn-adopt').click(function() {
        $.ajax({
            cache: true,
            type: "POST",
            url: './selectGasDealwith',
            data: { 
                'deviceid':$(this).attr("attrid"),
                'devicenum':$(this).attr("attrnum") 
            },
            async: false,
            error: function (request) {
                messageToast('error', '检索发生异常！', 3000);
