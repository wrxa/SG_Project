...
$(document).ready(function () {
    ...
    // 获得汽轮机（背压）页面初期数据
    getTurbineOfPtsByPlanId();

    // 给保存按钮绑定事件
    $('#SaveTurbineOfPtsData').bind('click', {flag: 0}, SaveTurbineOfPtsData);
    $('#SaveTurbineOfPtsData2').bind('click', {flag: 1},SaveTurbineOfPtsData);
    ...
    $('.left-select').bind('click', function(){
        ...
        if(moduleName != "t-4"){
            $('#btnDiv1')[0].style.display="none";
            $('#btnDiv2')[0].style.display="block";
        }else{
            $('#btnDiv2')[0].style.display="none";
            $('#btnDiv1')[0].style.display="block";
        }
    });

    createLabelBind("GPG_TurbineOfPtsForm", oldFormData);
...
//除尘除灰页面信息初期化
function getTurbineOfPtsByPlanId() {
    planId = $('#GPGCurrentPlanId').val();
    $.ajax({
        cache: true,
        type: "POST",
        url: './getTurbineOfPtsByPlanId',
        data: { "planId": planId },
        async: false,
        ...
        success: function (data) {
            assignmentForm(data.TurbineOfPtsJson);
            // if(data.TurbineOfPtsJson.s_parameter_flg == '1') {
            //     bool_input = true;
            // }
            var dataformInit = $("#GPG_TurbineOfPtsForm").serializeArray();  
...
function assignmentForm(datas){
    ...
    // if(datas['e_steam_extraction_select'] != '' && datas['e_steam_extraction_select'] != null){
    //     $("[name='e_steam_extraction_select']").val(datas['e_steam_extraction_select'].toFixed(0));
    // }
}
...
function getElements() {  
    var tagElements = $("#GPG_TurbineOfPtsForm input");  
...
//提交表单信息
function SaveTurbineOfPtsData(event) {
    // var s_parameter_flg = $("[name='s_parameter_flg']").val();
    ...
    if ($("[name='s_steam_type_test']").val() == '0' || $("[name='s_steam_type_test']").val() == null){
        ...
        $.ajax({
            cache: true,
            type: "POST",
            url: './SaveTurbineOfPtsData',
            data: $('#GPG_TurbineOfPtsForm').serialize(),
            async: false,
            error: function (request) {
            ...
            success: function (data) {
                if(data.newDatas == null){
                    ...
                    messageToast('success', '煤气发电-汽轮机计算数据保存成功！',2000);
                    // 刷新页面
                    getTurbineOfPtsByPlanId();
                    var dataformInit = $("#GPG_TurbineOfPtsForm").serializeArray();  
                    oldFormData = JSON.stringify({ dataform: dataformInit });
                    createLabelBind("GPG_TurbineOfPtsForm", oldFormData);
                    // if(s_parameter_flg != "1"){
                    //     window.location.href="/GPG_TurbineOfPTS"
                    // }
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.temperature == null){
                    ...
                    messageToast('error', '输入数据有误，计算发生异常!', 3000);
                }else{
...
function byPressureEntropyBack() {
    if ($("[name='e_backpressure_pressure']").val() == null || $("[name='e_backpressure_pressure']").val() == ""){
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.enthalpy == null){
                    messageToast('error', '输入数据有误，转换发生异常!', 3000);
                }else if(data.enthalpy == "-1"){
                    messageToast('error', '输入数据有误，计算发生异常!', 3000);
                }else{
