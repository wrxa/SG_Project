...
$(document).ready(function () {
    ...
    getSteamWaterPipeDataByPlanId();

    $('#submitSteamWaterPipe').bind('click', submitSteamWaterPipe);
    ...
    createLabelBind("GPG_SteamWaterPipeForm", oldFormData);
});

function getSteamWaterPipeDataByPlanId() {
    planId = $('#GPGCurrentPlanId').val();
    $.ajax({
        cache: true,
        type: "POST",
        url: './getSteamWaterPipeDataByPlanId',
        data: { "planId": planId },
        async: false,
        ...
        success: function (data) {
            assignmentForm(data.SteamWaterPipeJson, "GPG_SteamWaterPipeForm");
            var dataformInit = $("#GPG_SteamWaterPipeForm").serializeArray();  
            oldFormData = JSON.stringify({ dataform: dataformInit });
        }
    });
}

function submitSteamWaterPipe() {
    $.ajax({
        cache: true,
        type: "POST",
        url: './GPG_SaveSteamWaterPipeData',
        data: $('#GPG_SteamWaterPipeForm').serialize(),
        async: false,
        error: function (request) {
        ...
        success: function (data) {
            if(data.newDatas == null){
                ...
                assignmentForm(data.newDatas, "GPG_SteamWaterPipeForm");
                var dataformInit = $("#GPG_SteamWaterPipeForm").serializeArray();  
                oldFormData = JSON.stringify({ dataform: dataformInit });
                createLabelBind("GPG_SteamWaterPipeForm", oldFormData);
                messageToast('success', '煤气发电-汽水管道数据保存成功！',2000);
            }         
        }
    });
}
...
function getElements() {
    var tagElements = $("#GPG_SteamWaterPipeForm input");
