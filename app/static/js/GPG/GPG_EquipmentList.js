...
$(document).ready(function () {
    ...
    // $("#getEquipmentList").bind('click', getGPGEquipmentList);

    getGPGEquipmentList();

    var dataformInit = $("#GPG_EquipmentListForm").serializeArray();  
    oldFormData = JSON.stringify({ dataform: dataformInit });
    createLabelBind("GPG_EquipmentListForm", oldFormData);
...
function getGPGEquipmentList(){
    $.ajax({
        url: '/getGPGEquipmentList',
        data: {},
        type: 'post',
        cache: false,
        dataType: 'json',
...
function analyzeEquipmentType(data, type){
    ...
    if(type == "5monitor-fire"){
        tbodyString += "<tr attrid='' class='alert alert-success alert-dismissible' role='alert'>"
        + "<td align='center'></td>"
    ...
    if(type == "4control-fire" || type == "5monitor-fire" || type == "6monitor-water"){
        tbodyString += "<tr attrid='" + type + "' class='alert alert-success alert-dismissible' role='alert'>"
        + "<td align='center'></td>"
        ...
        tbodyString += "<tr attrid='" + type + "' class='alert alert-success alert-dismissible' role='alert' attr_type='" + type + "'>"
        + "<td align='center'></td>"
        + "<td align='center'></td>"
...
function insertRow(index, type){  
    /*
    var indexNumber = parseInt(index) + parseInt(2);
    var myTable = document.getElementById("equipment_list_table");
    var newRow = myTable.insertRow(indexNumber);
    ...
    cell1.innerHTML="<input class='form-control' type='text' readonly='true'/>";
    cell2.innerHTML="<input name='newName' class='form-control name' type='text'/>";
    cell3.innerHTML="<input name='newContent' class='form-control content' type='text'/>";
    cell4.innerHTML="<input name='newUnit' class='form-control unit' type='text'/>";
    cell5.innerHTML="<input name='newCount' class='form-control count' type='text'/>";
    cell6.innerHTML="<input name='newRemark' class='form-control remark' type='text'/>";
    */
